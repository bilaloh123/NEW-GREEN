...
const FRIGOS = ["Frigo 4","Frigo 5","Frigo 6","Frigo 7"];
  ...
  return(
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:22}}>
      {FRIGOS.map(f=>{const lts=inFrigo(f);const pal=lts.reduce((s,l)=>s+(l.nbPalettes||0),0);const kg=lts.reduce((s,l)=>s+(l.poidsNet||0),0);return(
        <div key={f} onClick={()=>setSelFrigo(f)} style={{background:selFrigo===f?"rgba(99,102,241,0.18)":"rgba(255,255,255,0.04)",border:"2px solid "+(selFrig
